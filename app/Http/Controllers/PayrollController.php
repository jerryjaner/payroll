    ...
    public function submit_payroll(Request $request){
        ...
        if($validator -> fails()){
            ...
            $RDND = DB::table('attendances')
                        ...
                        ->where('date', '>=', $fromdate)
                        ->where('date', '<=', $todate)
                        ->sum(DB::raw("TIME_TO_SEC(RDND_hours)")); // Calcuting the total hours and convert it to seconds
            $RDND_total_work_hour = gmDate("H", $RDND); 
            $RDND_total_min = gmDate("i", $RDND);

            
            //dd($RDND_total_work_hour);
            ...
            $SHND = DB::table('attendances')
                        ...
                        ->where('date', '>=', $fromdate)
                        ->where('date', '<=', $todate)
                        ->sum(DB::raw("TIME_TO_SEC(SHND_hours)")); // Calcuting the total hours and convert it to seconds
            ...
            $RHND = DB::table('attendances')
                        ...
                        ->where('date', '>=', $fromdate)
                        ->where('date', '<=', $todate)
                        ->sum(DB::raw("TIME_TO_SEC(RHND_hours)")); // Calcuting the total hours and convert it to seconds
            ...
            $RDSHND = DB::table('attendances')
                        ...
                        ->where('date', '>=', $fromdate)
                        ->where('date', '<=', $todate)
                        ->sum(DB::raw("TIME_TO_SEC(RDSHND_hours)")); // Calcuting the total hours and convert it to seconds
            ...
            $RDRHND = DB::table('attendances')
                        ...
                        ->where('date', '>=', $fromdate)
                        ->where('date', '<=', $todate)
                        ->sum(DB::raw("TIME_TO_SEC(RDRHND_hours)")); // Calcuting the total hours and convert it to seconds
            ...
            if($request -> schedule_shift == 'Day'){
                ...
                if($request -> monthly_rate == "Fixed Rate"){
                    ...
                    $payroll -> leave_without_pay = $request->lwp * $request -> daily_rate;
                    $payroll -> suspended = $request->suspended * $request -> daily_rate;
                    ...
                    $payroll -> total_deduction = 0.00   +  $payroll -> cash_advance +  $payroll -> deduct_adjustment +  $payroll -> leave_without_pay +  $payroll -> suspended;
                  ...
                  //  $payroll -> net_pay =  ($payroll -> employee_base_salary  +   $payroll -> allowance) - $payroll -> total_deduction; 
                    ...
                    //For the 13 month
                    //wala pa yung VTO 
                    $a_lwp_s = ($payroll -> late_undertime + $payroll -> employee_absent + $payroll -> leave_without_pay + $payroll -> suspended);
                    $computation = $payroll -> employee_base_salary - $a_lwp_s;
                    $payroll-> thirteen_month = $computation / 12;  
                    ...
                    //For the 13 month
                    //wala pa yung VTO 
                    $a_lwp_s = ($payroll -> late_total + $payroll -> undertime_total + $payroll -> employee_absent + $payroll -> leave_without_pay + $payroll -> suspended);
                    $computation = $payroll -> employee_base_salary - $a_lwp_s;
                    $payroll-> thirteen_month = $computation / 12;  
                        ...
                        foreach($pagibig_deduction as $pagibig_deduct){
                            ...
                            if ($payroll -> employee_base_salary >= $pagibig_deduct->monthly_salary_from && $payroll -> employee_base_salary <= $pagibig_deduct->monthly_salary_to) {
                                ...
                                $payroll -> total_deduction =  $payroll -> philhealth_deduction + $payroll -> sss_deduction +  $payroll -> pag_ibig_deduction + $payroll -> late_undertime + $payroll -> cash_advance + $payroll -> employee_absent +  $payroll -> deduct_adjustment 
                                                              + $payroll -> leave_without_pay +  $payroll -> suspended;
           ...
           //--------------------- Night SHIFT HERE ------------------------------------------
                ...
                if($request -> monthly_rate == "Fixed Rate"){
                    ...
                    $late_total = ($total_latehours * 60) + $late_min;
                    $late = $late_total / 60;
                    $computed_late = $late * $request -> rate_per_hour * 1; //Computed late

                   
                    //------------------ undertime Computation -----------------------------------------------//
                    $undertime_total = ($total_undertime * 60) + $undertime_min;
                    $undertime = $undertime_total / 60;
                    $computed_undertime = $undertime * $request -> rate_per_hour * 1; //Computed undertime

                    //------------------ Restday Computation -----------------------------------------------//
                    $restday_total_MinutesWorked = ($RDND_total_work_hour * 60) + $RDND_total_min;
                    $restday_hoursWorked = $restday_total_MinutesWorked / 60;
                    $restday_salary = $restday_hoursWorked * $request -> rate_per_hour * 1.3 * 0.1; //Computed Restday
                
                    //------------------ Rest Day and Special Holiday Computation ------------------------//
                    $RDSH_total_MinutesWorked = ($RDSHND_total_work_hour * 60) + $RDSHND_total_min;
                    $RDSH_hoursWorked = $RDSH_total_MinutesWorked / 60;
                    $RDSH_salary = $RDSH_hoursWorked * $request -> rate_per_hour * 1.5 * 0.1 * 8; //REST DAY SPECIAL HOLIDAY COMPUTATION
                    
                
                    //------------------ Rest Day Regular Holiday -------------------------------------//
                    $RDRH_total_MinutesWorked = ($RDRHND_total_work_hour * 60) + $RDRHND_total_min;
                    $RDRH_hoursWorked = $RDRH_total_MinutesWorked / 60;
                    $RDRH_salary = $RDRH_hoursWorked * $request -> rate_per_hour * 2.6 * 0.1 *8; //REST DAY REGULAR HOLIDAY COMPUTATION

                    
                    //------------------ Special Holiday -------------------------------------//
                    $SH_total_MinutesWorked = ($SHND_total_work_hour * 60) + $SHND_total_min;
                    $SH_hoursWorked = $SH_total_MinutesWorked / 60;
                    $SH_salary = $SH_hoursWorked * $request -> rate_per_hour * 1.3 * 0.1 * 8; //SPECIAL HOLIDAY COMPUTATION
                    

                    //------------------ Regular Holiday -------------------------------------//
                    $RH_total_MinutesWorked = ($RHND_total_work_hour * 60) + $RHND_total_min;
                    $RH_hoursWorked = $RH_total_MinutesWorked / 60;
                    $RH_salary = $RH_hoursWorked * $request -> rate_per_hour * 2.0 * 0.1 * 8;  //REGULAR HOLIDAY COMPUTATION
